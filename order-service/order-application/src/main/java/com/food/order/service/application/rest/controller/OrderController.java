package com.food.order.service.application.rest.controller;
...
public class OrderController {
    ...
    public ResponseEntity<TrackOrderResponse> getOrderByTrackingId(@PathVariable UUID trackingId) {
        TrackOrderResponse trackOrderResponse = orderApplicationService.trackOrderOrder(
                TrackOrderQuery.builder()
                        .orderTrackingId(trackingId)
                        .build()
        );

        log.info("Returning order status by tracking Id: {}", trackOrderResponse.getOrderTrackingId());
