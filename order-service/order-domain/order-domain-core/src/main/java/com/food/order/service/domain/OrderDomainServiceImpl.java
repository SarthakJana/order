...
public class OrderDomainServiceImpl implements OrderDomainService {
    ...
    public OrderCreatedEvent validateAndInitiateOrder(Order order, Restaurant restaurant) {
        ...
        setOrderAndProductInformation(order, restaurant);
        order.validateOrder();
        order.initializeOrder();
        log.info("Order initiated with order Id: {}", order.getId().getValue());
    ...
    private void validateRestaurant(Restaurant restaurant) {
        if (!restaurant.isActive()) {
            throw new OrderDomainException("Restaurant0 with id " + restaurant.getId().getValue() + " is currently not active");
        }
    }

    private void setOrderAndProductInformation(Order order, Restaurant restaurant) {
